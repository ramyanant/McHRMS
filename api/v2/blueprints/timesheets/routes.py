...
def my_timesheets():
    ...
    rows = db_rows("""SELECT t.*, s.name as status, t.project as project_name, c.name as client_name
        FROM timesheets t
...
def submit_my_timesheet():
    ...
    cur_obj.execute("""INSERT INTO timesheets
        (employee_id, client_id, project, week_ending,
         regular_hours, overtime_hours, total_hours, bill_rate,
         status_id, notes, submitted_at)
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW()) RETURNING id""",
        (emp_id, d.get('client_id'),
         d.get('project', d.get('notes', '')),
         d['week_ending'], total, 0, total, 0,
...
def list_timesheets():
    ...
    if proj_id: where.append("t.project=%s");    params.append(proj_id)
    ...
    rows  = db_rows(f"""SELECT t.*, s.name as status,
        ...
        t.project as project_name, c.name as client_name
        FROM timesheets t
        LEFT JOIN master_timesheet_statuses s ON s.id=t.status_id
...
def pending_approvals():
    ...
    team_ids = [r['id'] for r in db_rows(
        "SELECT id FROM employees WHERE reporting_manager_id=%s AND is_active=1", (emp_id,))]
    if not team_ids:
